...
// Definimos la interfaz para las props de la tarjeta interna
...
// 👇 AQUÍ ESTABA EL ERROR: Debe ser 'export const', no 'export default'
    ...
    return (
        <div style={{ padding: '20px', paddingBottom: '100px' }}>
            <div style={{textAlign:'center', marginBottom:'20px'}}>
                ...
                <p style={{fontSize: '13px', color: '#aaa'}}>Get points instantly & Bonus.</p>
            </div>
            
            <PackCard title="Starter" basePoints="100k" price="0.15 TON" color="#00F2FE" onClick={() => buyPack('starter')} />
            <PackCard title="Pro" basePoints="500k" bonusText="+5%" price="0.75 TON" color="#4CAF50" onClick={() => buyPack('pro')} />
            <PackCard title="Whale" basePoints="1M" bonusText="+10%" price="1.50 TON" color="#FFD700" onClick={() => buyPack('whale')} />
            <PackCard title="TYCOON 👑" basePoints="5M" bonusText="+20% FREE" price="7.50 TON" color="#E040FB" isBestValue={true} onClick={() => buyPack('tycoon')} />
...
const PackCard: React.FC<PackCardProps> = ({ title, basePoints, bonusText, price, color, isBestValue, onClick }) => (
    <div className="glass-card" style={{
        ...
        margin: '0 0 10px 0', padding: '15px', position: 'relative'
    }}>
        ...
        <div style={{marginBottom:'10px'}}>
            ...
            <div style={{color: '#fff', fontSize:'18px', fontWeight:'bold'}}>{basePoints}</div>
            {bonusText && <div style={{fontSize:'10px', color: '#4CAF50', fontWeight:'bold'}}>{bonusText}</div>}
        </div>
        <button className="btn-neon" style={{ width:'100%', padding: '8px', fontSize:'12px', background: isBestValue ? `linear-gradient(45deg, ${color}, #00F2FE)` : undefined, border: 'none', color: isBestValue ? 'white' : 'black' }} onClick={onClick}>
            {price}
        </button>
    </div>
);