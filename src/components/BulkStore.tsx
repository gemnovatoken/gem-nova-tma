...
// 1. Importaciones limpias (Solo lo que se usa)
import { Zap, Crown, Hexagon, Rocket, Shield, Cpu, Lock, Play } from 'lucide-react';
...
export const BulkStore: React.FC = () => {
    ...
    const buyPack = async (pack: string) => {
        ...
        else if (pack === 'whale') msg = "Confirm Whale: 1,050,000 Pts (Includes +5% Bonus) for 1.50 TON";
        else if (pack === 'tycoon') msg = "Confirm Tycoon: 5,350,000 Pts (Includes +7% Bonus) for 7.50 TON";
        else if (pack === 'emperor') msg = "Confirm Emperor: 18,700,000 Pts (Includes +10% Bonus) for 25.00 TON";
        else if (pack === 'blackhole') msg = "Confirm Black Hole: 80,500,000 Pts (Includes +15% Bonus) for 100.00 TON";
        ...
        <div className="cyber-bg" style={{ minHeight: '100%', paddingBottom: '120px', paddingTop: '20px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px', padding: '0 20px', position: 'relative', zIndex: 2 }}>
                ...
                <PackCard 
                    ...
                    price="0.75 TON" color="#4CAF50" icon={<Cpu size={16}/>} 
                    onClick={() => buyPack('pro')} 
                />
...
const CyberBigCard: React.FC<BigCardProps> = ({ title, points, bonus, price, color, icon, isLocked, onUnlock, onClick, isGodMode }) => (
    <div className="cyber-card" style={{ 
        ...
        {isLocked ? (
             ...
             // 👇 AQUÍ ESTABA EL ERROR (Corregido: solo un 'color')
